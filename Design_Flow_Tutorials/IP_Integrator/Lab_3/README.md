...
For Part 1, we create a basic Microblaze block design with the ZCU102 Evaluation Board.

In Part 2, we use the design created in part 1 to explore the Register Transfer Level (RTL) module reference feature of IP integrator.

In Part 3, we explore the RTL inference feature of module reference technology based on Microblaze design created in part 1 and RTL design added in part 2.

In part 4, we explore the new feature introduced for module reference called X_MODULE_SPEC, which combines the existing X_INTERFACE_INFO and X_INTERFACE_PARAMETER HDL attributes. This part
...
1.  Open the AMD Vivado™ Integrated Design Environment (IDE). Create a new
...
module reference in the sections below. You are required to recreate
...
7.  Now run connection automation and you see that design canvas
...
However, for this lab we keep design_1\_wrapper as the top design.
...
1.  Inferring Generics/Parameters in an RTL Module: If the source RTL contains generics or parameters, those are inferred at the time the module is added to the block design, and can also be configured in the re-customize Module Reference dialog box for a selected module.
    ...
    If you open the RTL code of axi_slave.v by double clicking on it under the sources tab, you can see the usage of parameter for DATA_WIDTH and ID_WIDTH
...
5.  In some cases, you may need to specify the order in which interfaces are inferred rather than letting the tools automatically infer them. The Module Reference feature lets you prioritize the order of the interface inference. There are several attributes that you can use to infer interfaces.
...
**::ipx::package_module_spec** Tcl command that has been added in Vivado 2022.1 to help users create, validate and convert an X_MODULE_SPEC attribute for use in any IP, BD or RTL shim module.
...
10. To check if the instantiation has been done properly in the shim.v file, you can use the Tcl command below to validate the module specification with respect to the target \'shim\' module.
...
1.  A ModuleRef must not contain AMD Zynq™ Processing System, CIPS and NOC IP.

2.  A ModuleRef may not instantiate one or more DCP modules.

3.  A ModuleRef may not instantiate one or more ModuleRefs (that is, \"Nested ModuleRefs\").
...
5.  RTL within one or more BD designs: Currently
    ...
    this case the you need to run ipx::package_module_spec on both
