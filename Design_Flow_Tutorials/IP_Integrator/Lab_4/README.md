...
When using a AMD Versal™ device, the IP integrator creates a Block Design
(BD) that contains a CIPS block (Control, Interfaces & Processing
System) and the NoC (Network-On-Chip) with DDR-MC (DDR Memory
Controller).

An AXI-based RTL module is imported using IP integrator Module Reference to
demonstrate how you can quickly add their own RTL to a block design. A
second RTL based IP that is packaged as an IP repository is also added to the Block Design.
...
4.  Once the “Block Automation” process is complete, the AXI NoC now appears on the IP integrator  Canvas configured for DDR4 (the DDR-MC is part
  of the NoC instance) with an external port named “CH0_DDR_0\_0” and
  connections to the CIPS through multiple AXI ports along with associated
...
Next, the CIPS is customized for QSPI and SD Card Boot Modes. We
...
1.  The IP Repository for the “wave_gen_v1_0” IP must first be added to IP Catalog. First open IP Catalog (IPC):
