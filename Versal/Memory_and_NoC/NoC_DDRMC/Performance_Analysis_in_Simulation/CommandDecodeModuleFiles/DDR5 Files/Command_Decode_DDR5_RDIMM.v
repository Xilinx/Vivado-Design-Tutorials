...
module Command_Decode_DDR5_RDIMM(
input CK_t,
input CS,
input [6:0] CA,
output writePattern,
...
// Capture CA0..CA6 on rising edge
always @(posedge CK_t) begin
	ca_rise <= CA;
	ca_rise_valid <= 1;
end

// Capture CA7..CA13 on falling edge and decode full CA[13:0]
always @(negedge CK_t) begin
	...
	if (ca_rise_valid) begin
		...
		if (CS && pending_act_cycle2) begin
		...
		else if (CS && pending_mrw_cycle2) begin
			OP[0] <= ca_rise[0];
			OP[1] <= ca_rise[1];
			OP[2] <= ca_rise[2];
			OP[3] <= ca_rise[3];
		...
		else if (CS && pending_wrp_group_cycle2) begin
			col[2] <= ca_rise[0];
			col[3] <= ca_rise[1];
			col[4] <= ca_rise[2];
			col[5] <= ca_rise[3];
		...
		else if (CS && pending_wr_group_cycle2) begin
			col[2] <= ca_rise[0];
			col[3] <= ca_rise[1];
			col[4] <= ca_rise[2];
			col[5] <= ca_rise[3];
		...
		else if (CS && pending_rd_group_cycle2) begin
			col[2] <= ca_rise[0];
			col[3] <= ca_rise[1];
			col[4] <= ca_rise[2];
			col[5] <= ca_rise[3];
			...
			else if (ca_rise[5] && !ca_rise[4] && ca_rise[3] && !ca_rise[2] && !ca_rise[1] && ca_rise[0]) begin
				BA[0] <= ca_rise[6];
				BA[1] <= CA[0];
				BG[0] <= CA[1];
				BG[1] <= CA[2];
			...
			else if (!ca_rise[4] && !ca_rise[3] && ca_rise[2] && !ca_rise[1] && ca_rise[0]) begin
				modeRegWrite <= 1;
