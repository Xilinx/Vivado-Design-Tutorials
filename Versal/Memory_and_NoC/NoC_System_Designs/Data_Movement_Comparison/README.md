<table class="sphinxhide" width="100%">
 <tr width="100%">
    <td align="center"><img src="https://github.com/Xilinx/Image-Collateral/blob/main/xilinx-logo.png?raw=true" width="30%"/><h1>Versal™ NoC System Design Tutorials</h1>
...
This tutorial uses a complex design example to demonstrate how the Versal&trade; network on chip (NoC) simplifies the design process for on-chip data movement. For comparison, a similar design is built in a Zynq&trade; UltraScale+&trade; device. The NoC frees up programmable logic resources that are consumed by SmartConnect in the Zynq UltraScale+ design. The Versal-based design can be run in hardware and you can measure data movement and power consumption.  
...
The *k*-means kernel is generated from C++ source code using Vitis&trade; HLS. The source code is provided in [hls/kmeans_vitis.cpp](hls/kmeans_vitis.cpp). To generate the kernel:
```bash
cd build/kernel
make
```
...
To build the top-level design, start the Vivado&trade; Design Suite in the `build/design` directory. Within the Vivado Tcl Console:
```tcl
...
A set of scripts is provided to facilitate running the design. This script assumes you are an internal AMD user accessing a board-farm board with systest. Future versions of this tutorial will include scripts to facilitate running the design on a standalone board.

To run the design within AMD:
```bash
cd cmd
