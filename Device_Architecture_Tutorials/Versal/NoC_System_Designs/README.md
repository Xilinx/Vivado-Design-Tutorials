...
The project can be built from script referencing solely from the source files contained within this repo. Note there are several block diagrams within the project - each block diagram is represented by a TCL script that is called from the main script.
...
- The NoC simulation model is generated using the `generate_switch_network_for_noc` TCL command
  - The `generate_switch_network_for_noc` TCL command assumes the ***Design Sources*** top-level is to be used for NoC model generation; this causes issues here as
    - There are multiple block diagrams with NoC access potential
...
This DCMAC/NoC reference is based on the 400G DCMAC example project - many of the generated example project files are untouched but captured in this repo for convenience.  
If desired, the original 400G DCMAC example project can be generated for reference purposes by running the following command in the `scripts` directory:
