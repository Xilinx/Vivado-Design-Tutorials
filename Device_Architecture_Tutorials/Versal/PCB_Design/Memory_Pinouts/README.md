...
The role of a schematic designer is in some cases separated from the system design, and as such, there can be a level of non-familiarity with system-level design tools, such as the AMD Vivado&trade; Design Suite, which are geared towards higher levels of system design.
...
This document guides the schematic designer, or anyone with a lower level of familiarity with the Vivado Design Suite, to obtain and verify memory pinouts for AMD Versal&trade; devices.    The steps shown are detailed, yet minimal, as the primary goal is to obtain and verify pinouts without concern for the higher-level design flows that the Vivado tools offer.
...
>**NOTE:** This tutorial assumes that you have access to an installed version of the Vivado Design Suite and appropriate licenses. For information regarding Installation and Licensing, refer to the *Vivado Design Suite User Guide: Release Notes, Installation, and Licensing* ([UG973](https://www.xilinx.com/cgi-bin/docs/rdoc?v=latest;t=vivado+install+guide)).
...
10. After a few moments a Diagram window will appear. Click the **+** sign to **add IP**.
...
![AXI General 2](images/axi_general_2.png)
    ...
    You will then notice five new tabs to the right of the QoS tab.
...
17. Click **Run Block Automation** to create the necessary logic required for the interface.
![Run Block Automation](images/run_block_automation.png)

18. In the "Run Block Automation" window, ensure that "Control, Interface, and Processing System" is clicked, and enable "1" AXI Traffic Generator, then click **OK**.
![Configure Run Block Automation](images/configure_run_automation.png)

    After a few moments, the block diagram will reappear with the CIPS block and traffic generator blocks in place.
![After Run Block Automation](images/after_block_automation.png)

19. Click **Run Connection Automation** to ensure all open ports get connected properly  
![Run Connection Automation](images/run_connection_automation.png)

20. In the "Run Connection Automation" window, click **All Automation**, which will automatically select all of the blocks that require connections to be made. Then click **OK**.
![Configure Connection Automation](images/configure_connection_automation.png)

21. After a few moments, the block diagram will update with connections made, which will introduce a clocking wizard block (clk_wiz) that will also require connection automation.    Again, click **Run Connection Automation**.
![clk_wiz block](images/clk_wiz.png)

22. In the "Run Connection Automation" window, click **All Automation**, which will automatically select all of the blocks that require connections to be made. Then click **OK**.
![clk_wiz automation](images/clock_automation.png)

    The block diagram will update, with all required blocks and connections made.
![Full Block Diagram](images/full_block_diagram.png)

23. Click the **Validate Design** icon to ensure that the block diagram is complete.
![Validate Design](images/validate_1.png)

    After a few moments, the NoC window should appear, along with the message that validation is successful. Click **OK**.
![Validation OK](images/validate_2.png)

24. Click the **Save** icon to save the block diagram.
![Save Block Diagram](images/save_block_diagram.png)

    The design will now be prepared for synthesis and implementation.
...
27. Right-click the design source, and select **Create HDL Wrapper**.
...
![Elaborate Design OK](images/elaborate_design_ok.png)
    ...
    The Open Elaborated Design step will take a few minutes to complete.

     >**NOTE:** You might see a message about a missing top-level design. If so, click **Cancel**, and click **Open Elaborated Design** again.
![Status Window](images/status_window.png)

    After the Elaborated Design is opened (this may take a few minutes), a Package floorplan will appear.
...
33. Click the **DDRMC_X#Y#** tab that corresponds to the banks where the memory interface is desired (in this example, the DDRMC chosen will contain banks 700, 701, and 702). After selection, click **OK**.
...
1. In the Sources tab, click on the **Constraints** entry.
    ...
    >**NOTE:** If there is already a constraint file present, then it is recommended to remove it. Right-click the constraint file and select **Remove File from Project**, then click **OK**.
...
The AMD Vitis&trade; tutorials are supported only for the versions and specific steps and conditions outlined in the tutorials. If you have any technical questions on the subjects contained in these tutorials, post on the boards located at the Community Forums.
